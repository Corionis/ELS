...
java -jar %base%\..\ELS.jar -d debug --dry-run -p ..\meta\publisher.json -s  ..\meta\subscriber.json -T ..\meta\targets.json -m ..\output\%name%-Mismatches-%dtime%.txt -w ..\output\%name%-WhatsNew-%dtime%.txt -f ..\output\%name%-%dtime%.log
