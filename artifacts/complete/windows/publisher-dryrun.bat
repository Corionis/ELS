...
java -jar %base%\..\ELS.jar -c info -d debug --dry-run --remote P -p ..\meta\publisher.json -s  ..\meta\subscriber.json -t ..\meta\subscriber-targets.json -m ..\output\%name%-Mismatches-%dtime%.txt -W ..\output\%name%-WhatsNew-%dtime%.txt -f ..\output\%name%-%dtime%.log
